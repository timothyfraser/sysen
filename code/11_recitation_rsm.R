...
newdata %>% 
  mutate(
    predict(m, newdata = ., 
            se.fit = TRUE) %>%
      # Needs to return a data.frame, so use
           as_tibble())
# fit = yhat
# se.fit = standard error for that prediction
...
mlog = cookies %>% lm(
  formula = sqrt(yum) ~ poly(molasses, 2) +  poly(ginger, 2) + I(molasses * ginger)  )
glance(mlog)

# Now, our predictions and se come back in logs!
# We must simulate, backtransform, and recompute confidence intervals
newdata %>% 
  mutate(predict(mlog, newdata = ., se.fit = TRUE) %>% as_tibble())
...
newdata %>% 
  ...
  mutate(
    ...
    se = rnorm(n = 1000, mean = fit, sd = se.fit)^2 %>% sd()
  ) %>%
  ungroup() %>%
  mutate(lower = fit - se * qnorm(0.975),
         upper = fit + se * qnorm(0.975))
